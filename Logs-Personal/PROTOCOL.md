...
This folder is `Logs-Personal/`. `<subject>` must be **Personal**.
...
| `Logs-Personal/HEAD.xml` | Newest delta. Always read. Always overwrite on write. |
| `Logs-Personal/MANIFEST.txt` | One filename per line, oldest first, `HEAD.xml` last. |
| `Logs-Personal/YYYY-MM-DDTHHmmssZ-<hash>.xml` | Immutable dated entries. Never edit. Never delete. |
| `Logs-Personal/ID-Discussion/` | One XML file per message. |
...
1. `Logs-Personal/<dated>.xml` — the new body
2. `Logs-Personal/HEAD.xml` — **same** body
3. `Logs-Personal/MANIFEST.txt` — previous dated names + the new dated name + `HEAD.xml` last
...
1. `github___get_file_contents` owner `Stackocakes-Hub` repo `Tracker-Vault` path `Logs-Personal/MANIFEST.txt`
2. Same for `Logs-Personal/HEAD.xml`
...
One new XML per message in `Logs-Personal/ID-Discussion/`.
...
Commit **two** files (plus pictures if any):

1. `Logs-Personal/ID-Discussion/<TARGET-dated-hash>.xml`
2. Append that name to `Logs-Personal/ID-Discussion/MANIFEST.txt`

Pictures are never inside XML. Put files in:

- `Logs-Personal/ID-Discussion/Pictures/` for thread screenshots
- `Logs-Personal/Pictures/` for log/ticket screenshots

Then add a sibling tag: `<image name="safe-file.png"/>`. Allowed: png, jpg, webp, gif. No SVG. 2 MB max. 4 per post. Name is `[A-Za-z0-9._-]` only. Missing or bad pictures are ignored and do not break the scanner.
